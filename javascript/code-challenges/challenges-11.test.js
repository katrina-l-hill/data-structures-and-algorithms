...
This function should first remove any elements that are not numbers or are *wrong* not divisible by five.
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let arr = [];
  input.map(childArray =>{
    let divByFive = childArray.filter(item => typeof(item) === 'number' && item % 5 === 0)
    let twoPowers = divByFive.map(item => 2 ** item);
    arr.push(twoPowers);
  });
  return arr;
};
